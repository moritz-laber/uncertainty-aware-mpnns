...
correlation = sys.argv[1]                  # needs to be inif, dndf, indf
...
for nonlin_str, nonlinearity in nonlinearities:
    ...
    ## Train Model ##
    print(f'Training on {correlation} using l={num_layers} layers, h={num_hidden} hidden units, and {nonlin_str} nonlinearity.')
    ...
    # save trained model
    if save_result:
        eqx.tree_serialise_leaves(f'{result_dir}{correlation}/weights_model=gcn_corr={correlation}_nonlin={nonlin_str}_l={num_layers}_hidden={num_hidden}_reps={num_reps}.eqx', gcn)
    ...
    if save_result:
        with open(f'{result_dir}{correlation}/hyperparams_model=gcn_corr={correlation}_nonlin={nonlin_str}_l={num_layers}_hidden={num_hidden}_reps={num_reps}.pkl', 'wb') as f:
